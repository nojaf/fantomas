...
let rec (|Const|) c =
    ...
    | SynConst.String (s, kind, _) ->
        match kind with
        | SynStringKind.Verbatim -> sprintf "@%A" s
        | _ -> sprintf "%A" s
    | SynConst.Bytes (bs, kind, _) ->
        match kind with
        | SynByteStringKind.Verbatim -> sprintf "@%A" bs
        | _ -> sprintf "%A" bs
...
let (|Val|) (SynValSig (ats, (IdentOrKeyword (OpNameFullInPattern (s, _)) as ident), tds, t, vi, isInline, _, px, ao, _, _)) =
